    ...
    try:
        for acc in accountIds_list:
            ...
            if fb_resp != None:
                ...
                campaignExistence_flag,campaignExistence_resp = account_fbcampaignids_existence_status(tradbProd,acc,
                                                                                                       fbCampaignIds)
                if campaignExistence_flag == True:
                    fb_costs = fb_data_pre_processing(fb_resp,campaignExistence_resp)
                    final_response = final_exe_push_changes(fb_costs,campaignExistence_resp)
                    
                    return (campaignExistence_resp,final_response)
                else:
                    missing_fbCampaignIds_df = pd.DataFrame(campaignExistence_resp)
                    msg=("Test Alert! Ad Spent Service, Facebook CampaignId Don't Exist in Tradb Tags Collection:\n"+
                         missing_fbCampaignIds_df.to_html())
                    msg2=("Test Alert! Failed to Dump Ad Spend Data for Specific Account Mentioned below\n"+
                         "Please Impute Missing Tags Details Match for Facebook Camapigns\n"+
                         "FB Account ID"+" : "+str(campaignExistence_resp[0]["AccountId"])+"\n"+
                         "FB Account Name"+" : "+str(campaignExistence_resp[0]["AccountName"]))
                    ...
                    index_fb_campaign_missing_tags_details(esProd,
                                                           result,
                                                           FB_Campaign_TAG_FIX_INDEX)
                    return fb_resp
            else:
                print('Request Failed To fetch Data From Graph Api, Nothing to Insert In Database \n')
