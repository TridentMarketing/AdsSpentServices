...
def extract_tags_required_info(fbCampaignId,payload):
    ...
    try:
        ...
        tags_json["resort"]=str(getTradbResortInfo(tradbProd,str(payload.get("resort",""))))
...
def final_exe_push_changes(df,campaignsInfo_payload):
    ...
    status = check_on_date_and_ad_account_id(esDev,DEV_AD_SPEND_INDEX,dateCreated,ad_account_id)
    if status == True:
        try:
    ...
    #                 print("Error While Dumping Doc in MongoDb")
        ...
        print("Record already has been Dumped,"+
